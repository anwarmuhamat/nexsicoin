...
const char * const BITCOIN_CONF_FILENAME = "nexsicoin.conf";
...
#else
    const char* pszModule = "nexsicoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nexsicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nexsicoin";
#else
    // Unix
    return pathRet / ".nexsicoin";
#endif
#endif
}
